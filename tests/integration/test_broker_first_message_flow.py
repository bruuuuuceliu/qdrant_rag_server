"""Broker-first message flow smoke test."""
...
from task_manager_service import InMemoryTaskStateRepository
...
async def test_ingest_flow_stores_raw_indexes_chunks_and_publishes_final_result(tmp_path) -> None:
    ...
    task_manager = TaskManagerDispatcher(
        producer=bus,
    ...
    await project.handle(bus.pop(TOPICS.domain_project_commands))
    await task_manager.dispatch_domain_result(bus.pop(TOPICS.domain_project_results))
    await ingestion_handler.handle(bus.pop(TOPICS.helper_ingestion_commands))
    first = await task_manager.finalize_helper_result(bus.pop(TOPICS.helper_ingestion_results))
    await storage.handle(bus.pop(TOPICS.helper_storage_commands))
    second = await task_manager.finalize_helper_result(bus.pop(TOPICS.helper_storage_results))
    await retrieval_index.handle(bus.pop(TOPICS.helper_retrieval_index_commands))
    final_result = await task_manager.finalize_helper_result(
        bus.pop(TOPICS.helper_retrieval_index_results)
    )

    status = await status_store.get_status(accepted.task_id)
    final = bus.pop(TOPICS.task_results)
    ...
    assert final.payload["result"] == status.result


@pytest.mark.asyncio
async def test_manager_to_task_manager_to_project_to_retrieval_to_status_flow() -> None:
    bus = EnvelopeBus()
    status_store = InMemoryTaskStatusStore()
    manager = ManagerService(task_producer=bus)
    ...
    await task_manager.finalize_helper_result(bus.pop(TOPICS.helper_retrieval_results))
    ...
    assert final.task_id == accepted.task_id
    assert final.payload["status"] == "completed"


@pytest.mark.asyncio
async def test_task_manager_waits_for_retrieval_and_storage_results(tmp_path) -> None:
    bus = EnvelopeBus()
    status_store = InMemoryTaskStatusStore()
    state_repository = InMemoryTaskStateRepository()
    manager = ManagerService(task_producer=bus)
    ...
    await task_manager.dispatch_intake(bus.pop(TOPICS.task_intake))
    await project.handle(bus.pop(TOPICS.domain_project_commands))
    domain_result = bus.pop(TOPICS.domain_project_results)

    await task_manager.dispatch_domain_result(domain_result)

    await retrieval.handle(bus.pop(TOPICS.helper_retrieval_commands))
    first = await task_manager.finalize_helper_result(bus.pop(TOPICS.helper_retrieval_results))
    await storage.handle(bus.pop(TOPICS.helper_storage_commands))
    final_result = await task_manager.finalize_helper_result(bus.pop(TOPICS.helper_storage_results))
