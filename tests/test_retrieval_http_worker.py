...
class RetrievalHttpRunnerTest(unittest.TestCase):
    ...
    def test_local_runner_references_retrieval_http_worker(self) -> None:
        ...
        self.assertIn("--external-retrieval-http", run_all)
        self.assertIn("retrieval_service.server.worker", run_all)
        self.assertIn("PYTHON_BIN", run_all)
        self.assertIn("MANAGER_RETRIEVAL_CLIENT_MODE=http", run_all)
        self.assertIn("requires manager project planning to stay local", run_all)
        self.assertIn('mkdir -p "$RUNTIME_DIR" "$LOG_DIR"', run_all)
        self.assertIn("docker info", run_all)
        self.assertIn("wait_for_service_port", run_all)
        self.assertIn("did not open", run_all)
        self.assertIn("retrieval-http.pid", stop_all)
        self.assertIn("retrieval-http.log", readme)
        self.assertIn("--reset --init --external-retrieval-http", readme)
        self.assertIn("Do not combine it with", readme)
