"""Retrieval-index helper broker server tests."""
...
@pytest.mark.asyncio
async def test_retrieval_index_helper_server_runs_once() -> None:
    handler = Handler()
    consumer = Consumer(_command())
    context = RetrievalIndexHelperServerContext(handler=handler, consumer=consumer)

    await context.run_once()

    assert consumer.topic == TOPICS.helper_retrieval_index_commands
    assert handler.handled == [consumer.envelope]


@pytest.mark.asyncio
async def test_retrieval_index_helper_server_start_stop() -> None:
    context = RetrievalIndexHelperServerContext(handler=Handler(), consumer=Consumer(_command()))
...
def _command() -> MessageEnvelope:
    return MessageEnvelope.create(
        ...
        payload={
            ...
            "plan": {"collection_name": "docs", "chunks": []},
        },
    )
