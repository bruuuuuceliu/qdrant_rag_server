"""Retrieval helper broker server tests."""
...
@pytest.mark.asyncio
async def test_retrieval_helper_server_runs_once() -> None:
    handler = Handler()
    consumer = Consumer(_command())
    context = RetrievalHelperServerContext(handler=handler, consumer=consumer)

    await context.run_once()

    assert consumer.topic == TOPICS.helper_retrieval_commands
    assert handler.handled == [consumer.envelope]


@pytest.mark.asyncio
async def test_retrieval_helper_server_start_stop() -> None:
    context = RetrievalHelperServerContext(handler=Handler(), consumer=Consumer(_command()))
