...
class ManagerAppContext:
    ...
    async def shutdown(self) -> None:
        ...
        if self.ingestion_app is not None:
...
async def create_app(
    ...
    *,
    ...
    ingestion_client = ProjectDocumentIngestionClient(project_client)
    if ingestion_app is not None and ingestion_app.jobs is not None:
        ingestion_client = LocalIngestionClient(
            ingestion=project_client,
            jobs=ingestion_app.jobs,
        )
    retrieval_client = ProjectDocumentRetrievalClient(project_client)
    if project_app is not None:
        ...
        retrieval_client = ProjectPlannedRetrievalApiClient(
